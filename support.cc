/*
 ...
 $Id: support.cc,v 1.21.2.1 2010/11/19 20:09:29 rhuey Exp $
...
//handle multiple ligands -Huameng 11/18/2007 
//Quat Genotype::readQuat()
Quat Genotype::readQuat(int ilig)
{
    Quat q;  
    int gene_start_idx = gene_index_lig[ilig][0]; 
     
	q.x = gread(gene_start_idx + 3).real;
	q.y = gread(gene_start_idx + 4).real;
	q.z = gread(gene_start_idx + 5).real;
	q.w = gread(gene_start_idx + 6).real;
	// q = convertQuatToRot( q );
    
    return q;   
}

void Genotype::writeQuat( Quat q, int ilig )
{
    //handle multiple ligands -Huameng 11/18/2007 
    int gene_start_idx = gene_index_lig[ilig][0];   
    write( q.x, gene_start_idx + 3 );
    write( q.y, gene_start_idx + 4 );
    write( q.z, gene_start_idx + 5 );
    write( q.w, gene_start_idx + 6 );

}

//handle multiple ligands -Huameng 11/18/2007
//Quat Phenotype::readQuat()
Quat Phenotype::readQuat(int ilig)
{
    Quat q;
    int gene_start_idx = gene_index_lig[ilig][0];
    q.x = gread(gene_start_idx + 3).real;
...
//handle multiple ligands -Huameng 11/18/2007
void Phenotype::writeQuat( Quat q, int ilig )
{  
    int gene_start_idx = gene_index_lig[ilig][0];
    write( q.x, gene_start_idx + 3 );
    write( q.y, gene_start_idx + 4 );
...
#ifdef DEBUG
   (void)fprintf(logFile, "support.cc/Phenotype::Phenotype(unsigned int init_number_of_dimensions=%d, Representation **init_value_vector), number_of_points=%d\n",
   		init_number_of_dimensions, number_of_points);
#endif /* DEBUG */
...
#ifdef DEBUG
   (void)fprintf(logFile, "support.cc/Phenotype::Phenotype(const Phenotype &original) number_of_dimensions=%d number_of_points=%d\n", 
   		number_of_dimensions, number_of_points);
#endif /* DEBUG */
...
// Converts phenotype to mol's state and returns this individual's mol data.
    ...
    State molState;
    Molecule molcopy;
	//handle multi-ligands -Huameng 11/12/2007
