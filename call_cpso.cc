...
State call_cpso(Local_Search * LocalSearchMethod, State sInit, int n_exec, int S, int D, 
...
nb_eval=0;
	...
	//for (nb_eval=1; nb_eval < eval_max; nb_eval++)
	//{
	do
	{
		if (init_links == 1)
        ...
        //LocalSearchMethod=new Pseudo_Solis_Wets1(ntor, max_its, max_succ, max_fail, 2.0, 0.5, search_freq, rho, lb_rho);
		copyState2Dimension(&Xi[sb], sbNew[s]);
