/*
 ...
 $Id: investigate.cc,v 1.15.2.1 2010/11/19 20:09:30 rhuey Exp $
...
/*  Initialize random number generator with a time-dependent seed...  */
    ...
    for ( Test = 0;  Test < maxTests;  Test++ ) {
        ...
        for (LocalTest = 0; LocalTest < NumLocalTests; LocalTest++, Test++ ) {
            ...
            do { /* while (rms > MaxRms); */
                do { /* while (B_outside); */
                    if (mode == RANDOM_MODE) {
                        ...
                        sNow = mkRandomState( nlig, ntor, F_TorConRange, N_con, info);
                        if (outlev > 2) {
            ...
            e = trilinterp( 0, natom, crd, charge, abs_charge, type, map, info, ALL_ATOMS_INSIDE_GRID, ignore_inter, NULL_ELEC, NULL_EVDW, NULL_ELEC_TOTAL, NULL_EVDW_TOTAL, NULL)
