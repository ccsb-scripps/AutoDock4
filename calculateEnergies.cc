/*
 ...
 $Id: calculateEnergies.cc,v 1.3.2.1 2010/11/19 20:09:30 rhuey Exp $
...
extern double  nb_elec_total;
extern double  nb_vdm_hb_total;
extern double  nb_desol_total;
...

{
    ...
    eb.e_inter_moving_fixed = trilinterp( 0, true_ligand_atoms, tcoord, charge, abs_charge, type, map, 
                         ...
                         &elec_total, &vdm_hb_total, &desol_total);
                         
    eb.e_total_elec += elec_total;
    eb.e_total_vdm_hb += vdm_hb_total;
    eb.e_total_desol += desol_total;
    
    *p_emap_total =  vdm_hb_total + desol_total;
    *p_elec_total = elec_total;
    
    if (B_have_flexible_residues) {
        // computing trilinear-interpolated energies from atom = true_ligand_atoms to atom 
        // gives the intramolecular energy of flex residues within the protein
    ...
    if (ntor > 0) {
        ...
        eb.e_total_elec += nb_elec_total;
    	eb.e_total_vdm_hb += nb_vdm_hb_total;
    	eb.e_total_desol += nb_desol_total;
                                   ...
                                   Real unbound_internal_FE )
{
    ...
    eb->e_total_vdm_hb = 0.0;
    eb->e_total_elec = 0.0;
    eb->e_total_desol = 0.0;
}
