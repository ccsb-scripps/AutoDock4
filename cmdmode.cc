/*
 ...
 $Id: cmdmode.cc,v 1.20.2.1 2010/11/19 20:09:29 rhuey Exp $
...
*/
			   ...
			   for(i = 0; i < S.nlig; i++) {
                sscanf(command, "%*s %lf %lf %lf %lf %lf %lf %lf", 
                    ...
                    &(S.Q[i].nx), &(S.Q[i].ny), &(S.Q[i].nz),  &(S.Q[i].ang));
                pr(logFile, "COMMAND: eval %lf %lf %lf %lf %lf %lf %lf\n         ", 
                    S.T[i].x, S.T[i].y, S.T[i].z, S.Q[i].nx, S.Q[i].ny, S.Q[i].nz, S.Q[i].ang);
                /**/
